...
i.
```{r}
p3 <- ggplot(df, aes(x=Age, y=MVPA)) + geom_point(color ='blue', alpha=0.5) + theme_dark()
plot(p3)
```
...
p3 <- p3+  geom_abline(intercept = coefficients(lm.fit)[1], slope = coefficients(lm.fit)[2],
              colour = "red", size = 0.4, linetype = "solid") + theme(axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16))
plot(p3)
